...
export default ({
  ...
  React.useEffect(() => {
    (async () => {
      const { result, total } = await authedApi.postStaffList({ data: { priority_list: group.item.map(_id => Number(_id)) }, page: filter.page + 1, keyword: filter.keyword, limit, access: 1 })
